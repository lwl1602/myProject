...
@if(count($errors) > 0)
    ...
    @endif
        <form class="form form-horizontal" id="form-article-add" action="{{ url('admin/index/create') }}" method="POST">
        <div class="row cl">
            <label class="form-label col-xs-4 col-sm-2">图片简介：</label>
            <div class="formControls col-xs-8 col-sm-9">
                <input type="text" class="input-text" value="" placeholder="" id="" name="img_intro">
            </div>
        </div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">首页图片上传：</label>
			<div class="formControls col-xs-8 col-sm-9">
...
<script type="text/javascript" src="{{ asset('style/lib/webuploader/0.1.5/webuploader.min.js') }}"></script>
<script type="text/javascript">
    
    
    /*$(function () {
        $('#filePicker-2').diyUpload();
    });*/
    ...
    (function( $ ){
        ...
        $(function() {
            ...
            uploader = WebUploader.create({
                ...
                dnd: '#dndArea',
                paste: '#uploader',
                swf: '{{ asset('style/lib/webuploader/0.1.5/Uploader.swf') }}',
                chunked: false,
                chunkSize: 512 * 1024,
                server: '{{ url('admin/index/upload') }}',
                // runtimeOrder: 'flash',
            ...
            function writeObj(obj){     //答应js中对象的相关信息
                var description = "";
                for(var i in obj){
                    var property=obj[i];
            ...
            function addFile( file ) {
                var imgurl = Math.floor(Math.random()*100000)+file.name;
                file.name = imgurl;
                var $li = $( '<li id="' + file.id + '">' +
                                '<input type="hidden" name="img[]" value="upload/'+file.name+'" />'+
                                '<p class="imgWrap"></p>'+
                ...
                if ( file.getStatus() === 'invalid' ) {
                    ...
                    uploader.makeThumb( file, function( error, src ) {
                        ...
                        if( isSupportBase64 ) {
                            input = $('<input type="text" name="img_'+file.id+'" value="upload'+file.name+'">');
                            img = $('<img src="'+src+'">');
                            $wrap.empty().append( input );
                            $wrap.empty().append( img );
                        } else {
                            $.ajax('{{ asset('sytle/lib/webuploader/0.1.5/server/preview.php') }}', {
                                method: 'POST',
                                data: src,
                                dataType:'json'
