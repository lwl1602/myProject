...
@section('content')
	...
	@endif
	<form action="{{ url('admin/sc/edit/'.$sc->sc_id)}}" method="post" class="form form-horizontal" id="form-admin-role-add" enctype="multipart/form-data">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>业务图标：</label>
			<div class="formControls col-xs-8 col-sm-9"> <span class="btn-upload form-group">
				<input class="input-text upload-url" type="text" name="uploadfile" id="uploadfile" readonly placeholder="业务的点击图标" style="width:200px" value="{{ $sc->sc_url }}">
		...
		<div class="row cl" id="showImg">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<img src="{{ asset($sc->sc_url) }}" class="thumbnail" alt="没有找到图片">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>旧图标预览：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="{{ $sc->sc_title }}" placeholder="" id="name" name="title">
			</div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9"> <span class="select-box">
				<select class="select" size="1" name="type">
					@for($i = 0;$i < count($bcs);$i++)
						<option value="{{ $bcs[$i]->bc_id }}"  @if ($sc->bc_id == $bcs[$i]->bc_id) selected @endif>{{ $bcs[$i]->bc_name }}</option>
					@endfor
				</select>
				</span> </div>
		</div>
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="content" style="width:800px;height:400px;visibility:hidden;">{{ $sc->sc_content }}</textarea>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				<a class="btn btn-primary radius" href="{{ url('admin/sc') }}" name="submit">&nbsp;返&nbsp;回&nbsp;列&nbsp;表&nbsp;</a>
