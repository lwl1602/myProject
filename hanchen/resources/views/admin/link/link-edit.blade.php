...
<article class="page-container">
	...
	@endif
	<form action="{{ url('admin/link/'.$link->link_id.'/edit')}}" method="get" class="form form-horizontal" id="form-admin-role-add">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>链接名：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="link_name" id="" placeholder="显示在主界面信息" value="{{ $link->link_name }}" class="input-text">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>链接地址：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" name="link_url" id="" placeholder="注意链接地址" value="{{ $link->link_url }}" class="input-text">
			</div>
		</div>
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2">链接描述：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<textarea name="link_describe" cols="" rows="" class="textarea"  placeholder="说点什么...最少输入10个字符，最多200字" datatype="*10-100" dragonfly="true" nullmsg="备注不能为空！" onKeyUp="$.Huitextarealength(this,200)">{{ $link->link_describe }}</textarea>
